...
const CoverRender = (props) => {
    ...
    return (
        <div className={`overflow-hidden ${displayimage ? `block` : `hidden`}`}>
            <Image alt="Project cover, an Image with the project title." className="relative w-full  object-cover rounded-t-md"  src={cover_url} width={props.width} height={props.height} onError={() => setDisplayimage(false)}></Image>
        </div>
    );
};
